                ...
                if ($indexOfAuthorEnd !== false && (mb_strlen(
                    ...
                    for (
                        $repeatingStringLength = min(
                            20,
                            mb_strlen($fullContent) - $indexOfAuthorEnd
                        ); $repeatingStringLength >= 20; $repeatingStringLength--
                    ) {
    ...
    public function getResponseByMessage(Message $message): string
    {
        ...
        if (str_starts_with($incomingMessageText, '/personality')) {
            ...
            if ($personality === 'reset' || $personality === '') {
                ...
                return 'Теперь я буду тебе отвечать как Виктор89';
            }
...
//            $previousMessageUserName = str_replace(' ', '_', $previousMessage->userName);
        ...
        for ($i = 0; $i < 5; $i++) {
            if ($this->doesResponseNeedTobeRegenerated($response, $prompt)) {
                $response = $this->getCompletion($incomingMessageConvertedToPrompt, $personality);
            } else {
                break;
            }
        }
