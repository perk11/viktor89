...
LIMIT :limit"
    ...
    {
        $statement = $this->sqlite3Database->prepare('DELETE FROM persona WHERE name = :name');
        $statement->bindValue(':name', $name);
        $statement->execute();
    }
    ...
    {
        $statement = $this->sqlite3Database->prepare('SELECT * FROM persona WHERE name = :name');
        $statement->bindValue(':name', $name);
        $result = $statement->execute();
    ...
    {
        $statement = $this->sqlite3Database->prepare('SELECT * FROM persona ORDER BY name');
        $result = $statement->execute();
        $personas = [];
        while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
    ...
    {
        $statement = $this->sqlite3Database->prepare('SELECT COUNT(1) FROM persona WHERE user_id = :user_id');
        $statement->bindValue(':user_id', $userId);
        $result = $statement->execute();
