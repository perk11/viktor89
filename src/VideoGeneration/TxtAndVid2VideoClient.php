...
class TxtAndVid2VideoClient
{
    ...
    public function generateByPromptAndVid(string $videoContent, string $prompt, int $userId): VideoApiResponse
    {
        $params = $this->getParamsBasedOnUserPreferences($userId);
        $params['init_videos'] = [base64_encode($videoContent)];
        $params['prompt'] = $prompt;
        $response = $this->request('vid_txt2vid', $params);
    ...
    /**
     * The user's selected video-edit model, falling back to the first configured
     * model when there is no preference or it is unknown.
     */
