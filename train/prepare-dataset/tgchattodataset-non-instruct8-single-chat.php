<?php
if (count($argv) !== 4) {
	echo "Usage: " . $argv[0] . " bot_user_name result.json output.jsonl\n";
	exit(1);
}
...
foreach ($messagesByIndex as $messageIndex => $message) {
    ...
    if ($chain[count($chain) - 1]['from'] !== $botUserName) {
        //Skip chains that do not end on a bot message
        continue;
    }
    $rowText = '';
    foreach ($chain as $chainMessage) {
        $author = $chainMessage['from'] === $botUserName ? 'bot' : 'human';
