...
app = Flask(__name__)
parser = argparse.ArgumentParser(description="Inference server for https://github.com/huchenlei/ComfyUI-layerdiffuse using ComfyUI")
            ...
            response = {
                ...
                'info': json.dumps({
                    'infotexts': [f'{prompt}\nSteps: {steps}, Seed: {seed}, Size: {width}x{height}, Model: '
                                  + os.path.splitext(model)[0] + " layerdiffuse"
                                  ]
                })
            }
