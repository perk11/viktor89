...
class ContextCompactorTest extends TestCase
{
   ...
   public function testSummaryMessageIsUserMessage(): void
   {
       ...
       $result = $compactor->compact($ctx);
       // Summary is a user-role message so the conversation always starts with
       // system → user (summary), satisfying strict user/assistant alternation.
       $this->assertTrue($result->messages[0]->isUser);
   }
   ...
   // ─── progressive summarization & chunking ──────────────────────────────
   ...
   public function testProgressiveSummaryFeedsExistingSummaryIntoPrompt(): void
   {
       ...
       // The first message is an existing summary from a prior compaction.
       $summaryMsg = new AssistantContextMessage();
       $summaryMsg->isUser = true;
       $summaryMsg->text = ContextCompactorTest::summaryWrapper('The user likes cats.');

       $ctx = self::makeContext([
           ['isUser' => true, 'text' => 'old msg'],
           ['isUser' => true, 'text' => 'recent'],
       ]);
       array_unshift($ctx->messages, $summaryMsg);

       $compactor->compact($ctx);

       $this->assertNotEmpty($capturedPrompts);
       $this->assertStringContainsString('The user likes cats.', $capturedPrompts[0]);
       $this->assertStringContainsString('old msg', $capturedPrompts[0]);
   }
   ...
   public function testLargeConversationChunkedIntoMultipleSummaryCalls(): void
   {
       ...
       // Each "User: AAAAAAAAAA" is 15 chars serialized, so ~6 fit per 100-char chunk.
       $oldMessages = array_fill(0, 20, ['isUser' => true, 'text' => 'AAAAAAAAAA']);
       $ctx = self::makeContext(array_merge($oldMessages, [['isUser' => true, 'text' => 'recent']]));

       $result = $compactor->compact($ctx);

       // Should make multiple summary calls (chunked map-reduce).
       $this->assertGreaterThan(1, $callCount, 'Large conversation should be chunked into multiple summary calls');
       // Final result has 1 summary + 1 recent message.
   ...
   public function testProgressiveSummaryPassesPriorChunkResultToNextChunk(): void
   {
       ...
       // After the first chunk, subsequent prompts should contain the running summary.
       $this->assertCount(count($capturedPrompts), $capturedPrompts);
       if (count($capturedPrompts) > 1) {
           $this->assertStringContainsString('running summary', $capturedPrompts[1]);
       }
   }

   private static function summaryWrapper(string $summary): string
   {
