...
class EngineTest extends TestCase
{
    ...
    {
        // Message 48 is the /new reset and 49 the bot's confirmation reply:
        // history reading aborts at 48, and the confirmation itself must not
        // seed the fresh context either, so only the incoming message remains.
        $message = $this->buildMessage('hello there', 'private', 11111, messageId: 50);

        $confirmation = new InternalMessage();
        $confirmation->id = 49;
        $confirmation->chatId = 11111;
        $confirmation->userId = self::BOT_ID;
        $confirmation->userName = 'Viktor89';
        $confirmation->replyToMessageId = 48;
        $confirmation->messageText = 'Контекст сброшен, предыдущие сообщения больше не учитываются.';

        $recentMessages = [$confirmation];
        foreach ([48 => '/new@Viktor89Bot', 47 => 'msg47', 46 => 'msg46'] as $id => $text) {
            $m = new InternalMessage();
            $m->id = $id;
            $m->chatId = 11111;
            $m->messageText = $text;
        ...
        $this->assertNotNull($capturedChain);
        $this->assertSame([50], array_map(fn (InternalMessage $m) => $m->id, $capturedChain->getMessages()));
    }

    public function testPrivateChatKeepsBotMessageNotReplyingToNewCommand(): void
    {
        // Only the bot's reply to the /new command itself is dropped: a bot
        // message replying to something else is regular post-reset history.
        $message = $this->buildMessage('hello there', 'private', 11111, messageId: 50);

        $botReply = new InternalMessage();
        $botReply->id = 49;
        $botReply->chatId = 11111;
        $botReply->userId = self::BOT_ID;
        $botReply->userName = 'Viktor89';
        $botReply->replyToMessageId = 47;
        $botReply->messageText = 'как настроение?';

        $recentMessages = [$botReply];
        foreach ([48 => '/new', 47 => 'привет'] as $id => $text) {
            $m = new InternalMessage();
            $m->id = $id;
            $m->chatId = 11111;
            $m->messageText = $text;
    ...
    public function testPrivateChatKeepsUserReplyToNewCommand(): void
    {
        // Only the bot's confirmation is dropped: a genuine user reply to their
        // own /new command is the first message of the fresh conversation.
        $message = $this->buildMessage('hello there', 'private', 11111, messageId: 50);

        $userReply = new InternalMessage();
        $userReply->id = 49;
        $userReply->chatId = 11111;
        $userReply->userId = 222;
        $userReply->userName = 'Alice';
        $userReply->replyToMessageId = 48;
        $userReply->messageText = 'начнём заново!';

        $recentMessages = [$userReply];
        foreach ([48 => '/new', 47 => 'msg47'] as $id => $text) {
            $m = new InternalMessage();
            $m->id = $id;
            $m->chatId = 11111;
            $m->messageText = $text;
